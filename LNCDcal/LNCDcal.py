...
def time2gdict(dt,tzstr="America/New_York"): return({'dateTime': time2g(dt), 'timeZone': tzstr})
...
class LNCDcal():
  ...
  def find_in_range(self,dtmin,dtmax):
    if( type(dtmin) == datetime.datetime ):
       dtmin= time2g(dtmin)
    if( type(dtmax) == datetime.datetime ):
       dtmax= time2g(dtmax)
    ...
    items = [ calInfo(i) for i in events['items'] if i['start'].get('dateTime')]
    # check time
  ...
  def insert_event(self,startdt,dur_h,summary,desc):
    ...
    eventres = self.cal.events().insert(calendarId=self.calendarId,body=event).execute()
    return(eventres)
